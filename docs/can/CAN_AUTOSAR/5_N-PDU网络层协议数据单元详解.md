...
| FF+CF 多帧发送 | ✅ 已实现 | 标准流程：发 FF → 等 FC → 按 STmin/BS 节奏发 CF |
| FF+CF 多帧接收 | ✅ 已实现 | 含 FC 回复和缓冲区重组 |
| FC 流控状态机 | ✅ 已实现 | 发送方等 FC（WAIT_FC → SENDING_CF）；接收方回 FC(CTS) |
| 超时处理 | 🟡 部分 | **N_As**（SF 确认）/ **N_Bs**（等 FC）/ **N_Cr**（等 CF）已实现；N_Ar（接收确认）未实现 |
| 多通道并发 | ✅ 已支持 | 当前配置 2 个通道（TX + RX） |

> **超时参数说明**（都在 `CanTp_Cfg.h` 里，单位 ms）：N_As = 发出的 SF 等确认超时（1000ms）；N_Bs = 发完 FF 等 FC 超时（1000ms）；N_Cr = 收多帧时等下一帧 CF 超时（1000ms）。超时后自动放弃本次传输并回 IDLE——这是 ISO 15765-2 规定的最低容错要求，没有超时，一次丢帧就会卡死整个通道。
