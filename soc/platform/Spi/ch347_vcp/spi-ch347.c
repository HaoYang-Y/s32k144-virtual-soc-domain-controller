...
static int ch347_get_hw_config(struct ch347_spi *ch347) {
	...
	dev_info(&ch347->pdev->dev, "cfg: nss=%u cs_config=0x%02x reserved=%02x %02x %02x %02x polarity=%u phase=%u",
		ch347->cfg.nss, ch347->cfg.cs_config,
		ch347->cfg.reserved[0], ch347->cfg.reserved[1],
		ch347->cfg.reserved[2], ch347->cfg.reserved[3],
		ch347->cfg.polarity, ch347->cfg.phase);
	return 0;
}
...
static int ch347_transfer_setup(struct ch347_spi *ch347, u32 speed, u8 mode) {
	...
	ch347->cfg.nss = 1;                      /* 软件 CS: CS_CTRL 先于数据传输 */
	ch347->cfg.reserved[2] &= ~0xC0U;        /* CS1/CS2 均低电平有效 */
	ch347->cfg.write_read_interval = 5000U;  /* CS 到时钟间隔 5ms, 给 MCU 识别时间 */
...
{
	/* 仅在 SPI_NO_CS 模式下跳过 CS 控制 (原逻辑条件反了, CS 从不生效) */
...
static int ch347_transfer_one_message(struct spi_controller *controller, struct spi_message *m)
{
	...
	if (true || xfer->speed_hz != ch347->speed || (spi->mode & (SPI_CPHA|SPI_CPOL|SPI_LSB_FIRST)) != ch347->mode) {
		/* 强制每次传输前重新下发 SPI 配置 (含 nss CS 模式) */
	...
	/* CS 拉低后延时 5ms, 确保从机 (S32K144) 识别到片选再开始时钟 */
	msleep(5);
...
static int add_slave(struct ch347_spi *ch347, struct spi_board_info *board_info, uint8_t bits_per_word)
{
	...
	/* Sanity check */
	if (cs >= ch347->controller->num_chipselect)
		return -EINVAL;

	board_info->bus_num = ch347->controller->bus_num;

	if (ch347->slaves[cs] != NULL) {
		return -EADDRINUSE;
	}

	ch347->slaves[cs] = spi_new_device(ch347->controller, board_info);
	if (!ch347->slaves[cs]) {
		return -ENOMEM;
	}
...
static int ch347_spi_probe(struct platform_device *pdev)
{
	...
	ch347->controller = controller;
	ch347->controller->dev.of_node = dev->of_node;
	ch347->pdev = pdev;
	...
	controller->min_speed_hz = MIN_SPI_SPEED;
	controller->max_speed_hz = MAX_SPI_SPEED;
	controller->bits_per_word_mask = 0;

	controller->bus_num = -1;
	controller->mode_bits = SPI_CPOL | SPI_CPHA | SPI_LSB_FIRST; // SPI_CS_HIGH -- possible
